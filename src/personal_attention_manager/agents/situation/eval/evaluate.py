...
from personal_attention_manager.agents.situation.core.agent import SituationAgent
from personal_attention_manager.agents.situation.core.schemas import (
    Message,
    SituationInput,
    SituationType,
)
...
class EvalRow:
    ...
    expected_situation_type: SituationType
    notes: str
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            rows.append(
                EvalRow(
                    id=raw["id"],
                    input=SituationInput(recent_messages=messages),
                    expected_situation_type=SituationType(raw["expected_situation_type"]),
                    notes=raw.get("notes", ""),
                )
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--data",
        default=str(_script_dir / "generated_situation_examples.jsonl"),
        help="Path to JSONL eval dataset.",
    )
    parser.add_argument(
        "--model",
        default=None,
        help="Optional model override. If omitted, agent default is used.",
    )
    parser.add_argument(
        "--runs-dir",
    ...
    agent = SituationAgent(model=args.model)
    ...
    for row in rows:
        total += 1
        per_label_total[row.expected_situation_type] += 1

        t0 = time.perf_counter()
        prediction = agent.classify(row.input)
        latencies_ms.append((time.perf_counter() - t0) * 1000)

        actual = prediction.situation_type
        expected = row.expected_situation_type
    ...
    labels = list(SituationType)

    report = build_report_md(
        title="Situation Eval",
        run_id=run_id,
        model=args.model,
        data_path=args.data,
        total=total,
